...
export async function loadPasswords() {
  ...
  if (redis) {
    try {
      ...
      // Upstash normally returns an object, but older/alternate KV setups
      // can return the stored JSON as a string. Support both formats.
      let obj = data;
      ...
      if (obj && typeof obj === 'object' && !Array.isArray(obj)) {
        return Object.entries(obj)
          .filter(([name, password]) => String(name).length > 0 && password != null)
          .map(([name, password]) => ({
            name: String(name),
      ...
      // Only seed Redis from the environment when Redis is actually empty.
      const fromEnv = loadPasswordsFromEnv();
      if (fromEnv.length > 0) {
        const seeded = Object.fromEntries(fromEnv.map((e) => [e.name, e.password]));
      ...
      console.error('loadPasswords Redis error:', err);
...
export function findMatchingPassword(input, passwords) {
  // Passwords are compared exactly. Do not trim or otherwise modify them:
  // generated passwords may contain symbols such as $, #, +, =, etc.
  const value = String(input ?? '');
  for (const entry of passwords) {
    if (timingSafeEqual(value, String(entry.password))) return entry;
  }
  return null;
}
