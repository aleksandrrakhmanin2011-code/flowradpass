...
/** Читает список паролей из SITE_PASSWORDS_JSON */
...
/** Ищет пароль среди разрешённых (timing-safe) */
...
// Создаёт подписанный токен. При удалении/смене пароля этого человека старые токены умирают.
...
// Проверяет токен + что этот конкретный пароль всё ещё в списке
export async function verifyToken(token, secret, passwords) {
  ...
  // Пароль, которым вошли, должен всё ещё быть в списке
